...
public class ReservationService {
    ...
    public ResponseEntity<?> makeReservation(int passengerId, String flightLists) {
        ...
        Passenger passenger = passengerRepository.findFirstById(passengerId);
        ...
        if(fList.size() != flightL.size()){
            try {
                return new ResponseEntity<>(FlightService.getResponse("BadRequest", "400",
    ...
    public ResponseEntity<?> getReservation(Passenger passenger, String format) {
        ...
        if (reservation != null) {
            ...
            try {
                httpHeaders.setContentType(MediaType.APPLICATION_JSON);
                return new ResponseEntity<>(FlightService.getResponse("BadRequest", "400", "No Reservation details are found").toString(), HttpStatus.NOT_FOUND);
    ...
    public ResponseEntity<?> cancelReservation(int reservationNumber) {
        ...
        if (reservation != null) {
            ...
            httpHeaders.setContentType(MediaType.APPLICATION_XML);
            try {
                JSONObject response = FlightService.getResponse("", "200", "Reservation with number " + reservationNumber + " cancelled successfully.");
            ...
            httpHeaders.setContentType(MediaType.APPLICATION_JSON);
            try {
                JSONObject response = FlightService.getResponse("BadRequest", "404", "Reservation with number " + reservationNumber + " does not exist");
